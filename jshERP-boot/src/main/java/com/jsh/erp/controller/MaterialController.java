...
public class MaterialController extends BaseController {
    ...
    public TableDataInfo getList(@RequestParam(value = Constants.SEARCH, required = false) String search,
        ...
        List<MaterialVo4Unit> list = materialService.select(materialParam, standard, model, color, brand, mfrs,
                otherField1, otherField2,
                otherField3, weight, expiryNum, enableSerialNumber, enableBatchNumber, position, enabled, remark,
                categoryId, mpList);
    ...
    public String checkIsExist(@RequestParam("id") Long id, @RequestParam("name") String name,
            ...
            @RequestParam("otherField3") String otherField3, @RequestParam("unit") String unit,
    ...
    public JSONObject findBySelect(@RequestParam(value = "categoryId", required = false) Long categoryId,
        ...
        try {
            ...
            List<MaterialVo4Unit> dataList = materialService.findBySelectWithBarCode(categoryId, q,
                    StringUtil.toNull(standardOrModel),
                    StringUtil.toNull(color), StringUtil.toNull(brand), StringUtil.toNull(mfrs),
                    StringUtil.toNull(otherField1), StringUtil.toNull(otherField2), StringUtil.toNull(otherField3),
                ...
                for (MaterialVo4Unit material : dataList) {
                    ...
                    if (material.getUnitId() == null) {
                        ...
                        if (commodityUnit.equals(unit.getOtherUnitTwo()) && unit.getRatioTwo() != null) {
                            ratioStr = "[" + unit.getRatioTwo().stripTrailingZeros().toPlainString()
                                    + unit.getBasicUnit() + "]";
                        }
                        if (commodityUnit.equals(unit.getOtherUnitThree()) && unit.getRatioThree() != null) {
                            ratioStr = "[" + unit.getRatioThree().stripTrailingZeros().toPlainString()
                                    + unit.getBasicUnit() + "]";
                        }
                    }
    ...
    public JSONObject getMaterialByMeId(@RequestParam(value = "meId", required = false) Long meId,
        ...
        try {
            ...
            if (materialList != null && materialList.size() != 1) {
                ...
                MaterialName = MaterialName + expand
    ...
    public void exportExcel(@RequestParam(value = "categoryId", required = false) String categoryId,
        ...
        try {
            materialService.exportExcel(StringUtil.toNull(categoryId), StringUtil.toNull(materialParam),
                    StringUtil.toNull(color),
    ...
    public BaseResponseInfo getMaterialByBarCode(@RequestParam("barCode") String barCode,
        ...
        try {
            ...
            String[] mpArr = mpList.split(",");
            // 支持序列号查询，先根据序列号查询唛头，如果查不到就直接查唛头
                    ...
                    if ("QTCK".equals(prefixNo)) {
                        ...
                        if (organId == null) {
                            ...
                            if (systemConfigService.getCustomerStaticPriceFlag()) {
                                ...
                                BigDecimal lastUnitPrice = depotItemService.getLastUnitPriceByParam(organId,
    ...
    public BaseResponseInfo getListWithStock(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            List<MaterialVo4Unit> dataList = materialService.getListWithStock(depotList, idList,
                    StringUtil.toNull(position), StringUtil.toNull(materialParam),
                    moveAvgPriceFlag, zeroStock, StringUtil.safeSqlParse(column), StringUtil.safeSqlParse(order),
                    (currentPage - 1) * pageSize, pageSize);
            int total = materialService.getListWithStockCount(depotList, idList, StringUtil.toNull(position),
                    StringUtil.toNull(materialParam), zeroStock);
            MaterialVo4Unit materialVo4Unit = materialService.getTotalStockAndPrice(depotList, idList,
                    StringUtil.toNull(position), StringUtil.toNull(materialParam));
            map.put("total", total);
            map.put("currentStock",
                    materialVo4Unit.getCurrentStock() != null ? materialVo4Unit.getCurrentStock() : BigDecimal.ZERO);
            if (moveAvgPriceFlag) {
                map.put("currentStockPrice",
                        materialVo4Unit.getCurrentStockMovePrice() != null ? materialVo4Unit.getCurrentStockMovePrice()
                                : BigDecimal.ZERO);
            } else {
                map.put("currentStockPrice",
                        materialVo4Unit.getCurrentStockPrice() != null ? materialVo4Unit.getCurrentStockPrice()
                                : BigDecimal.ZERO);
            }
            map.put("currentWeight",
                    materialVo4Unit.getCurrentWeight() != null ? materialVo4Unit.getCurrentWeight() : BigDecimal.ZERO);
