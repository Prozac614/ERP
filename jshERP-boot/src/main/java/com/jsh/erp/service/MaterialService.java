...
public class MaterialService {
    ...
    public List<MaterialVo4Unit> select(String materialParam, String standard, String model, String color, String brand,
            String mfrs,
            String otherField1, String otherField2, String otherField3, String weight, String expiryNum,
        ...
        try {
            ...
            if (null != list && list.size() > 0) {
                ...
                for (MaterialVo4Unit m : list) {
                    ...
                    m.setInitialStock(
                            initialStockMap.get(m.getId()) != null ? initialStockMap.get(m.getId()) : BigDecimal.ZERO);
                    m.setBigUnitInitialStock(getBigUnitStock(m.getInitialStock(), m.getUnitId()));
                    m.setStock(
                            currentStockMap.get(m.getId()) != null ? currentStockMap.get(m.getId()) : BigDecimal.ZERO);
            ...
            if (obj.get("stock") != null) {
                ...
                for (int i = 0; i < stockArr.size(); i++) {
                    ...
                    if (jsonObj.get("id") != null && jsonObj.get("initStock") != null) {
                        ...
                        if (StringUtil.isNotEmpty(number) && Double.parseDouble(number) > 0 || lowSafeStock != null
                                || highSafeStock != null) {
                            insertInitialStockByMaterialAndDepot(depotId, mId, parseBigDecimalEx(number), lowSafeStock,
                                    highSafeStock);
    ...
    public List<MaterialVo4Unit> findBySelectWithBarCode(Long categoryId, String q, String standardOrModel,
            String color,
    ...
    public void exportExcel(String categoryId, String materialParam, String color, String materialOther, String weight,
        ...
        // 查询商品主唛头相关列表
        List<MaterialVo4Unit> dataList = materialMapperEx.exportExcel(materialParam, color, materialOther, weight,
                expiryNum, enabled, enableSerialNumber,
        ...
        String nameStr = "名称*,规格,型号,颜色,品牌,类别,基础重量(kg),保质期(天),基本单位*,副单位,基本唛头*,副唛头,比例,多属性," +
        ...
        if (null != dataList) {
            for (MaterialVo4Unit m : dataList) {
                ...
                objs[9] = otherMaterialMap.get(m.getId()) == null ? ""
                        : otherMaterialMap.get(m.getId()).getCommodityUnit();
                ...
                objs[13] = m.getSku();
                objs[14] = m.getPurchaseDecimal() == null ? ""
                        : m.getPurchaseDecimal().setScale(3, BigDecimal.ROUND_HALF_UP);
                objs[15] = m.getCommodityDecimal() == null ? ""
                        : m.getCommodityDecimal().setScale(3, BigDecimal.ROUND_HALF_UP);
                objs[16] = m.getWholesaleDecimal() == null ? ""
                        : m.getWholesaleDecimal().setScale(3, BigDecimal.ROUND_HALF_UP);
    ...
    public BaseResponseInfo importExcel(MultipartFile file, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            for (int i = 2; i < rightRows; i++) {
                ...
                String barCode = ExcelUtils.getContent(src, i, 10); // 基础唛头
                String manyBarCode = ExcelUtils.getContent(src, i, 11); // 副唛头
                ...
                // 批量校验excel中有无重复唛头（1-文档自身校验，2-和数据库里面的商品校验）
            ...
            for (MaterialWithInitStock m : mList) {
                ...
                List<Material> materials = getMaterialListByParam(m.getName(), m.getStandard(), m.getModel(),
                        m.getColor(), m.getUnit(), m.getUnitId(), basicBarCode);
                if (materials.size() == 0) {
                    materialMapperEx.insertSelectiveEx(m);
                    mId = m.getId();
                } else {
                ...
                for (Depot depot : depotList) {
                    ...
                    if (stock != null && stock.compareTo(BigDecimal.ZERO) != 0) {
                        ...
                        if (StringUtil.isNotEmpty(materialExtend.getSku())) {
                            throw new BusinessRunTimeException(ExceptionConstants.MATERIAL_SKU_BEGIN_STOCK_FAILED_CODE,
                                    String.format(ExceptionConstants.MATERIAL_SKU_BEGIN_STOCK_FAILED_MSG,
                                            materialExtend.getBarCode()));
                        }
                        buildChangeInitialStock(deleteInitialStockMaterialIdList, insertInitialStockMaterialList,
                                materialDepotInitialMap, mId, depotId, materialDepotKey, stock);
                    } else {
                    ...
                    if (billCount == 0) {
                        if (stock != null && stock.compareTo(BigDecimal.ZERO) != 0) {
                            buildChangeCurrentStock(deleteCurrentStockMaterialIdList, insertCurrentStockMaterialList,
                                    materialDepotCurrentMap, mId, depotId, materialDepotKey, stock);
                        } else {
                            if (initStock.compareTo(BigDecimal.ZERO) != 0) {
                                buildChangeCurrentStock(deleteCurrentStockMaterialIdList,
                                        insertCurrentStockMaterialList, materialDepotCurrentMap, mId, depotId,
                                        materialDepotKey, stock);
                            }
                        }
                    } else {
                        ...
                        buildChangeCurrentStock(deleteCurrentStockMaterialIdList, insertCurrentStockMaterialList,
                                materialDepotCurrentMap, mId, depotId, materialDepotKey, currentNumber);
                    }
                }
            }
            // 批量更新库存,先删除后新增
            ...
            logService.insertLog("商品",
                    new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_IMPORT).append(mList.size())
     ...
     * 构造初始库存的变化
     */
    private void buildChangeInitialStock(List<Long> deleteInitialStockMaterialIdList,
            List<MaterialInitialStock> insertInitialStockMaterialList,
            Map<String, String> materialDepotInitialMap, Long mId, Long depotId, String materialDepotKey,
            BigDecimal stock) {
     ...
     * 构造当前库存的变化
     */
    private void buildChangeCurrentStock(List<Long> deleteCurrentStockMaterialIdList,
            List<MaterialCurrentStock> insertCurrentStockMaterialList,
            Map<String, String> materialDepotCurrentMap, Long mId, Long depotId, String materialDepotKey,
            BigDecimal stock) {
     ...
     */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void insertOrUpdateMaterialExtend(JSONObject materialExObj, String type, String defaultFlag, Long mId,
        ...
        if (StringUtil.isExist(materialExObj.get(type))) {
            ...
            if (StringUtil.isNotEmpty(materialExtend.getSku())) {
                ...
                List<MaterialExtend> meList = materialExtendService.getListByMaterialIdAndDefaultFlagAndBarCode(mId,
                        "1", materialExtend.getBarCode());
                if (meList.size() == 0) {
     ...
     * @return
     */
    private List<Material> getMaterialListByParam(String name, String standard, String model, String color, String unit,
            Long unitId, String basicBarCode) throws Exception {
        List<Material> list = new ArrayList<>();
        MaterialExample example = new MaterialExample();
     ...
     */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void insertInitialStockByMaterialAndDepot(Long depotId, Long mId, BigDecimal stock, BigDecimal lowSafeStock,
            BigDecimal highSafeStock) {
        MaterialInitialStock materialInitialStock = new MaterialInitialStock();
        materialInitialStock.setDepotId(depotId);
        materialInitialStock.setMaterialId(mId);
    ...
    public String getMaxBarCode() {
        ...
        // 使用 Stream API 处理唛头列表
        OptionalLong maxBarcode = barCodeOldList.stream()
                .filter(StringUtil::isNumeric) // 过滤掉非数字唛头
    ...
    public List<MaterialVo4Unit> getListWithStock(List<Long> depotList, List<Long> idList, String position,
        ...
        List<MaterialVo4Unit> dataList = materialMapperEx.getListWithStock(depotList, idList, position, materialParam,
                zeroStock, column, order, offset, rows);
        for (MaterialVo4Unit item : dataList) {
            ...
            item.setInitialStock(
                    null != initialStockMap.get(item.getId()) ? initialStockMap.get(item.getId()) : BigDecimal.ZERO);
    ...
    public int getListWithStockCount(List<Long> depotList, List<Long> idList, String position, String materialParam,
            Integer zeroStock) {
        return materialMapperEx.getListWithStockCount(depotList, idList, position, materialParam, zeroStock);
    }

    public MaterialVo4Unit getTotalStockAndPrice(List<Long> depotList, List<Long> idList, String position,
            String materialParam) {
    ...
    public String getMaterialOtherByParam(String[] mpArr, MaterialVo4Unit m) {
        ...
        for (int i = 0; i < mpArr.length; i++) {
            if (mpArr[i].equals("自定义1")) {
                materialOther = materialOther + ((m.getOtherField1() == null || m.getOtherField1().equals("")) ? ""
                        : "(" + m.getOtherField1() + ")");
            }
            if (mpArr[i].equals("自定义2")) {
                materialOther = materialOther + ((m.getOtherField2() == null || m.getOtherField2().equals("")) ? ""
                        : "(" + m.getOtherField2() + ")");
            }
            if (mpArr[i].equals("自定义3")) {
                materialOther = materialOther + ((m.getOtherField3() == null || m.getOtherField3().equals("")) ? ""
                        : "(" + m.getOtherField3() + ")");
            }
        }
