-- ===========================
-- 库存计算简化修复脚本
-- 修复首页表格中的库存计算问题（简化版，避免语法错误）
-- ===========================

DELIMITER $$

DROP PROCEDURE IF EXISTS refresh_material_period_summary_simple$$

CREATE PROCEDURE refresh_material_period_summary_simple(IN tenant_id BIGINT)
BEGIN
    ...
    SELECT CONCAT('库存计算修复完成，处理时间：', NOW()) as result;
END$$

DELIMITER ;

-- 立即执行修复
CALL refresh_material_period_summary_simple(NULL);
