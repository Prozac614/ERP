...
DROP PROCEDURE IF EXISTS refresh_material_period_summary_minimal$$

CREATE PROCEDURE refresh_material_period_summary_minimal(IN tenant_id BIGINT)
BEGIN
    ...
    -- 上期结存：通过库存变动历史计算上期结束时的库存
    LEFT JOIN (
        SELECT
            di.material_id,
            dh.tenant_id,
            -- 计算上期结束时的库存：当前库存 - 本期净变动
            GREATEST(0,
                COALESCE(curr.current_stock, 0) -
                (COALESCE(curr_in.in_qty, 0) - COALESCE(curr_out.out_qty, 0))
            ) as previous_stock
        FROM (
            SELECT DISTINCT di.material_id, dh.tenant_id
            FROM jsh_depot_head dh
            INNER JOIN jsh_depot_item di ON dh.id = di.header_id
            WHERE dh.delete_flag = '0' AND di.delete_flag = '0'
        ) base
        LEFT JOIN (
            ...
            WHERE delete_flag = '0'
            GROUP BY material_id, tenant_id
        ) curr ON base.material_id = curr.material_id AND base.tenant_id = curr.tenant_id
        LEFT JOIN (
            SELECT di.material_id, dh.tenant_id, SUM(IFNULL(di.basic_number, 0)) as in_qty
            FROM jsh_depot_head dh
            INNER JOIN jsh_depot_item di ON dh.id = di.header_id
        ...
        ) curr_in ON base.material_id = curr_in.material_id AND base.tenant_id = curr_in.tenant_id
        LEFT JOIN (
            SELECT di.material_id, dh.tenant_id, SUM(IFNULL(di.basic_number, 0)) as out_qty
            FROM jsh_depot_head dh
            INNER JOIN jsh_depot_item di ON dh.id = di.header_id
            ...
            AND DATE(dh.oper_time) <= current_period_end
            GROUP BY di.material_id, dh.tenant_id
        ) curr_out ON base.material_id = curr_out.material_id AND base.tenant_id = curr_out.tenant_id
    ) ps ON m.id = ps.material_id AND (tenant_id IS NULL OR m.tenant_id = ps.tenant_id)

    -- 本期出库：直接查询本期时间范围内的出库记录
    LEFT JOIN (
        SELECT
            di.material_id,
            dh.tenant_id,
    ...
    SELECT 'Stock calculation fixed successfully' as result;
END$$
