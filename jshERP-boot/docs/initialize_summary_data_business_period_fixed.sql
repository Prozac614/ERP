...
-- 检查去年出库数据
SELECT 
    '去年出库单据数量' as description,
    COUNT(*) as count
FROM jsh_depot_head 
WHERE type = '出库' 
AND status = '1' 
AND delete_flag = '0'
AND YEAR(oper_time) = YEAR(CURDATE()) - 1;
...
-- 检查有库存的商品数量
SELECT 
    '有库存的商品数量' as description,
    COUNT(DISTINCT material_id) as count
FROM jsh_material_current_stock 
WHERE delete_flag = '0' 
AND current_stock > 0;

SELECT 'Business period data initialization completed successfully' as status; 